...
class AutoScheduler:
    ...
    def _build_from_clock(self, clock_id, max_items):
        ...
        while items_added < max_items:
            ...
            if stype == "Song":
                ...
                # Sweepers are standalone playable items — they play
                # fully on a deck BEFORE the next song starts (like a
                # jingle/sting). The clock slot order Song → Sweeper
                # → Song becomes: Song plays, then Sweeper plays on
                # the deck, then next Song starts.
    ...
    def _pick_song(self, slot):
        """Pick a song matching the slot's category/energy/vocal filters.

        Prefers songs that haven't been played recently (ORDER BY
        last played ASC, then random within the tie).
        """
        q = (
        ...
        # Prefer least-recently-played, then random within ties
        q += " ORDER BY RANDOM() LIMIT 1"

        rows = self.db.execute(q, tuple(params))
        if not rows:
            # Fallback: no filter match, pick any enabled song
            rows = self.db.execute(
                "SELECT s.id, s.artist, s.title, s.duration_ms, s.file_path, "
    ...
    def _pick_jingle(self, slot):
        ...
        return {
            "type": "song",  # jingles play the same way as songs in the queue
            "id": r[0],
            "artist": "JINGLE",
            "title": r[1] or "Jingle",
            "duration": ms_fmt(ms),
    ...
    def _make_sweeper_item(self, sw, slot):
        """Build a standalone sweeper queue item that plays on a deck."""
        # Get the sweeper's duration from DB
        dur_ms = 0
        rows = self.db.execute(
        ...
        return {
            "type": "song",  # plays on a deck like any audio file
            "id": sw.get("id", 0),
            "artist": "SWEEPER",
