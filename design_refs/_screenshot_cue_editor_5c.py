"""Phase 5-C checkpoint screenshot — Audio Cue Editor complete:
options bar + metadata + AI banner + DB save flow."""
...
def main():
    ...
    out = os.path.join(ROOT, "screenshots", "audio_cue_editor_complete.png")
    ...
    def _capture():
        ...
        if state["dialog"] is not None:
            # Force the dialog to its design size (920×760) so all sections
            # render even on screens where BaseDialog's 92%-cap would scroll.
            dlg = state["dialog"]
            from ui.dialogs.base_dialog import SHADOW_PAD
            full_w = 920 + SHADOW_PAD * 2
            full_h = 740 + SHADOW_PAD * 2
            dlg.setMaximumSize(full_w + 200, full_h + 200)
            dlg.resize(full_w, full_h)
            app.processEvents()
